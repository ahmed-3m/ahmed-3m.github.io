 ...
 *
 * Quick contract:
 *  - Append new objects to `newsItemsArray`. Do not reorder (sorting is done
 *    at read time by date, newest first). `parseNewsItems` is the only way to
 ...
 *  - `url` must be the canonical source link, unique after canonicalization.
 *  - `date` is a real calendar ISO 'YYYY-MM-DD', not in the future (UTC).
 *  - Unknown fields on an item or on `take` are rejected.
 */
...
const newsItems = parseNewsItems(newsItemsArray)
