...
const copy = {
  ...
  faultrixEyebrow: { en: 'Previous project · 2025-2026', de: 'Vorheriges Projekt · 2025-2026', fr: 'Projet précédent · 2025-2026', es: 'Proyecto anterior · 2025-2026', ar: 'مشروع سابق · 2025-2026' },
  faultrix: {
    en: 'AI-powered construction quality control SaaS. Analyzed building photos and generated ONORM-compliant reports in under 1 minute - SHA-256 evidence chain, DSGVO compliant, AES-256 encryption. Built solo from zero to production.',
    de: 'KI-gestütztes SaaS für Bauqualitätskontrolle. Analysierte Baustellenfotos und erstellte ONORM-konforme Berichte in unter 1 Minute – SHA-256 Beweiskette, DSGVO-konform, AES-256 Verschlüsselung. Solo von null bis Produktion gebaut.',
    fr: "SaaS de contrôle qualité construction par IA. Analysait les photos de chantier et générait des rapports conformes ONORM en moins d'une minute - chaîne de preuve SHA-256, conforme DSGVO, chiffrement AES-256.",
    es: 'SaaS de control de calidad en construcción con IA. Analizaba fotos de obra y generaba informes compatibles con ONORM en menos de 1 minuto - cadena SHA-256, cumplimiento DSGVO, cifrado AES-256.',
    ar: 'منصة SaaS لمراقبة جودة البناء بالذكاء الاصطناعي. كانت تحلل صور المواقع وتنشئ تقارير متوافقة مع ONORM خلال أقل من دقيقة مع سلسلة أدلة SHA-256 وتشفير AES-256.',
  },
  ...
  mentoDesc: {
    en: 'A proactive personal-mentor assistant on Telegram that plans your week, sends morning briefings and evening reviews, and fires smart check-in triggers. Built on Supabase edge functions (Deno/TS) with a generic activity model (workouts, medication, study sessions, custom habits — no schema change per type), a deterministic generators→arbiter→composer decision engine for adaptive nudging, pgvector long-term memory with RAG, multi-provider LLM routing (GLM/Gemini/Groq) with provider failover, multimodal meal-photo vision, TTS voice reminders (Cloudflare MeloTTS), Google Calendar sync, optional Alexa Echo integration, invite-code gated beta access, and an ops dashboard with cost rollups. Now in beta — try it on Telegram at @sihem_ai_bot.',
    de: 'Ein proaktiver persönlicher Mentor-Assistent auf Telegram, der die Woche plant, Morgenbriefings und Abendreviews sendet und intelligente Check-Ins auslöst. Gebaut auf Supabase Edge Functions (Deno/TS) mit einem generischen Aktivitätsmodell (Workout, Medikation, Lernsessions, eigene Gewohnheiten — keine Schema-Änderung pro Typ), einer deterministischen Generatoren→Arbiter→Komponor-Entscheidungsmaschine für adaptives Nudging, pgvector-Langzeitgedächtnis mit RAG, Multi-Provider-LLM-Routing (GLM/Gemini/Groq) mit Provider-Failover, multimodaler Essen-Foto-Erkennung, TTS-Spracherinnerungen (Cloudflare MeloTTS), Google-Kalender-Sync, optionaler Alexa-Echo-Integration, Beta-Zugang per Einladungscode und einem Ops-Dashboard mit Kostenauswertungen. Jetzt in der Beta — test es auf Telegram unter @sihem_ai_bot.',
    fr: 'Un assistant mentor personnel proactif sur Telegram qui planifie votre semaine, envoie des briefings matinaux et des revues du soir, et déclenche des check-ins intelligents. Construit sur Supabase edge functions (Deno/TS) avec un modèle d\'activité générique (sport, médication, sessions d\'étude, habitudes personnalisées — sans changement de schéma par type), un moteur de décision déterministe generateurs→arbitre→compositeur pour le nudging adaptatif, mémoire long terme pgvector avec RAG, routage LLM multi-fournisseurs (GLM/Gemini/Groq) avec failover, vision multimodale des photos de repas, rappels vocaux TTS (Cloudflare MeloTTS), synchro Google Calendar, intégration Alexa Echo optionnelle, accès beta géré par code d\'invitation, et un tableau de bord ops avec suivi des coûts. Désormais en beta — essayez-le sur Telegram à @sihem_ai_bot.',
    es: 'Un asistente mentor personal proactivo en Telegram que planifica tu semana, envía briefings matutinos y revisiones nocturnas, y activa check-ins inteligentes. Construido sobre Supabase edge functions (Deno/TS) con un modelo de actividad genérico (ejercicio, medicación, sesiones de estudio, hábitos personalizados — sin cambio de esquema por tipo), un motor de decisión determinista generadores→árbitro→compositor para nudging adaptativo, memoria a largo plazo pgvector con RAG, enrutamiento LLM multiproveedor (GLM/Gemini/Groq) con failover, visión multimodal de fotos de comidas, recordatorios de voz TTS (Cloudflare MeloTTS), sincronización con Google Calendar, integración opcional con Alexa Echo, acceso beta por código de invitación, y un panel de ops con seguimiento de costos. Ahora en beta — pruébalo en Telegram en @sihem_ai_bot.',
    ar: 'مساعد موجّه شخصي استباقي على تيليجرام يخطط أسبوعك، ويرسل موجزات صباحية ومراجعات مسائية، ويطلق تنبيهات ذكية للتحقق. مبني على Supabase edge functions (Deno/TS) مع نموذج نشاط عام (التمارين، الأدوية، جلسات الدراسة، العادات المخصصة — بدون تغيير المخطط لكل نوع)، محرك قرار حتمي (مولدات→محكّم→مؤلف) للتنبيه التكيّفي، ذاكرة طويلة المدى pgvector مع RAG، توجيه LLM متعدد المزودين (GLM/Gemini/Groq) مع تبديل المزود، رؤية متعددة الوسائط لصور الطعام، تذكيرات صوتية TTS (Cloudflare MeloTTS)، مزامنة Google Calendar، تكامل اختياري مع Alexa Echo، وصول تجريبي برمز دعوة، ولوحة تشغيل مع تتبع التكاليف. الآن في النسخة التجريبية — جرّبه على تيليجرام عبر @sihem_ai_bot.',
  },
