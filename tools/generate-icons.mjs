...
/**
 * Pull the gradient defs and the #mark group straight out of icon.svg.
 *
 * These used to be duplicated here as a string constant, which meant editing
 * icon.svg silently updated only the favicons while apple-touch and the PWA
 * icons kept rendering the previous mark. Extracting keeps one source of truth.
 */
function extractMark(svg) {
  const defs = svg.match(/<defs>[\s\S]*?<\/defs>/);
  const mark = svg.match(/<g id="mark">([\s\S]*?)<\/g>/);

  if (!defs) throw new Error("icon.svg: no <defs> block found");
  if (!mark) throw new Error('icon.svg: no <g id="mark"> group found');
  if (/<g[\s>]/.test(mark[1])) {
    throw new Error(
      'icon.svg: #mark contains a nested <g>, so extraction truncated it. ' +
        "Flatten the group before regenerating."
    );
  }
 ...
 */
const fullBleed = (mark, scale = 0.62) => {
...
const mark = extractMark(rounded);
const maskable = fullBleed(mark, 0.62);
const appleTouch = fullBleed(mark, 0.84);
